 ...
 */
async function initializeSharedWorker(url: string, anonKey: string, options?: any) {
  try {
    updateStatus('CONNECTING' as RealtimeStatus, 'Connecting to Supabase SharedWorker...');
    ...
    updateStatus('ERROR' as RealtimeStatus, 'Failed to connect', error as Error);
 ...
 */
function setupOffscreenCanvas(canvas: OffscreenCanvas, width: number, height: number) {
  try {

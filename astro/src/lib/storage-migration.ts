...
export async function migrateAuthStorage(): Promise<void> {
  ...
  try {
    ...
    if (!hasOldDB) {
      console.log('[Migration] Old database does not exist, marking as migrated');
    ...
    if (oldData.size === 0) {
      ...
      console.log(`[Migration] Found ${oldData.size} items in old database`);
...
async function deleteDatabase(dbName: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    req.onerror = () => {
      console.error(`[Migration] Error deleting database ${dbName}:`, req.error);
      reject(req.error);
    };

    req.onblocked = () => {
      console.warn(`[Migration] Database ${dbName} deletion blocked, resolving anyway`);

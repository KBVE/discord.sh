...
function ensureClient(): SupaShared {
  if (typeof window === 'undefined') {
    throw new Error('Supabase is client-only');
  }
  if (!_supa) _supa = new SupaShared();
  return _supa;
}
...
/** Get the shared instance (make sure you called initSupa() somewhere first) */
export function getSupa(): SupaShared {
  return ensureClient();
}

...
export const ReactSupaRealtime: FC<RealtimeContainerProps> = ({
  ...
  // State
  const [status, setStatus] = useState<RealtimeStatus>('DISCONNECTED' as RealtimeStatus);
  const [subscriptions, setSubscriptions] = useState<Map<string, ChannelSubscription>>(new Map());
    ...
    (event: MessageEvent<WorkerOutboundMessage>) => {
      ...
      switch (message.type) {
        ...
        case 'SUBSCRIBE_SUCCESS' as WorkerMessageType:
          console.log(`Subscription ${message.payload.id} successful`);
          break;

        case 'SUBSCRIBE_ERROR' as WorkerMessageType:
          console.error(`Subscription ${message.payload.id} failed:`, message.payload.error);
        ...
        default:
          console.warn('Unknown worker message:', (message as any).type);
      }
    },
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (workerRef.current) {
        workerRef.current.postMessage({ type: 'TERMINATE' as WorkerMessageType });
   ...
   */
  useEffect(() => {
    if (status === ('CONNECTED' as RealtimeStatus) && initialSubscriptions.length > 0) {
    ...
    >
      ...
      {showStatusIndicator && (
        ...
        >
          ...
          <span className="status-subscriptions" style={{ marginLeft: 'auto' }}>
            {subscriptions.size} subscription{subscriptions.size !== 1 ? 's' : ''}
          </span>
      ...
      {import.meta.env.DEV && (
        ...
        >
          ...
          {lastConnected && (
            <div>Last Connected: {new Date(lastConnected).toLocaleTimeString()}</div>
          )}
        </div>
      )}
    </div>
